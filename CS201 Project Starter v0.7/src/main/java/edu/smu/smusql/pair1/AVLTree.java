...
public class AVLTree<K> {
    ...
    private void replaceParentWithChild(AVLNode<K> parent, AVLNode<K> child) {
        ...
        if (child != null) {
            child.setParent(grandparent); // Set the child's parent only if child is not null
        }
    }

    private AVLNode<K> deleteLeafOrSingleChildNode(AVLNode<K> toDelete) {
    ...
    private AVLNode<K> BSTDeletion(AVLNode<K> toDelete) {
        if (toDelete.numChildren() <= 1) {
            return deleteLeafOrSingleChildNode(toDelete);
        } else {
            return deleteTwoChildrenNode(toDelete);
        }
    }

    private AVLNode<K> deleteTwoChildrenNode(AVLNode<K> toDelete) {
        // Find replacement node - will not be the root
    ...
    private void replaceNode(AVLNode<K> newNode, AVLNode<K> oldNode) throws NullPointerException {
        ...
        if (oldNode.getParent() != null) {
            oldNode.getParent().updateChild(newNode, oldNode);
        }
    }
    ...
    public void printTree(AVLNode<K> node, String prefix, boolean isLeft, AVLNode<K> parent) {
        if (node != null) {
            ...
            System.out.print(prefix + (isLeft ? "├ L: " : "└ R: ") + node + " (Parent: "
                    + (node.getParent() != null ? node.getParent() : "null") + ", Height: " + node.getHeight() + ")\n");
    ...
    public void print() {
        if (root != null) {
            System.out.println("Root: " + root + ", Height: " + root.getHeight() + ", Parent: " + root.getParent());
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Delete: 80 6"); // 60 right is 78
        ageTree.remove(80, 6);
        ageTree.print();
        System.out.println();
        ...
        ageTree.remove(78, 11);
        ageTree.print();
        System.out.println();

        System.out.println("Delete: 99 12"); // list left 1 if false, 90 right is 94 if true
        ageTree.remove(99, 12);
        ageTree.print();
        System.out.println();
    }
